    ...
    public partial class ChatMessagePageModel : BasePageModel
    {
            ...
            : base(navigationService, true)
        {
            ...
            _connection = new HubConnectionBuilder()
                .WithUrl(Environments.URl_SERVER_HTTPS_DEVICE_4G_CHAT_HUB_MAC, options =>
                {
        ...
        [RelayCommand]
        async Task OpenProfileDetailAsync(object param)
        {
            if (OpenProfileDetailCommand.IsRunning || IsBusy)
                return;
            try
            {
                IsBusy = true;
                await NavigationService.PushToPageAsync<DetailProfilePage>(param: _partner.UserId, isPushModal: true);
                await Task.Delay(100);
            }
            finally
            {
